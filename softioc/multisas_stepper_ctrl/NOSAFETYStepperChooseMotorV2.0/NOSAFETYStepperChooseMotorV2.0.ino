...
int SpeedMotors = 5; // Higher = Slower. Full period is double this number in miliseconds (20 seems stable)
int StepperMultiplier = 4; // Stepper multiplier (Use if microstepping by the factor equal to [microsteps setting]/ 200).
  ...
  if (Serial.available() > 0)
  {
    ...
    if (NumMotors == 6) {
      ...
      for (int j = 0; j < abs(NumMotors); j++) {
        ...
        StepsPerRev[j] = (Serial.parseInt())*StepperMultiplier; // Determine number of steps to drive the motor
