 ...
 *  - default serial for the high bandwidth data comm
 *  - serial 1 (second port) for the slow control parameters
 *  
 *  Bram Slagmolen
 ...
 *  
 */
const String SOFTWARE_VERSION = "2020-03-28";
const String SOFTWARE_ID = "ncal_response_ds";
...
const float PERIOD_1HZ = 1000UL;                  // 1 Hz update period, in milliseconds
const float PERIOD_16HZ = 63UL;                   // 16 Hz update period, in milliseconds
const float PERIOD_32HZ = 31UL;                   // 32 Hz update period, in milliseconds
const float SAMPLE_PERIOD = PERIOD_16HZ;
...
void setup() {
  ...
  strcpy(arg1, "1");
  set_engage_driver("set engage driver");

  // set angular_frequency_setpoint = 1.1;
  strcpy(arg1, "1.1");
  set_acc_pull_frequency("set setpoint");
  
  // Sync Teensy Controller Time with computer/RPI,
  // Need user input before proceeding!
  // TZ_adjust=10; echo T$(($(date +%s)+60*60*$TZ_adjust))> /dev/cu.usbmodem79315001
  // TZ_adjust=10; echo T$(($(date +%s)+60*60*$TZ_adjust))> /dev/ttyACM0
...
*/
  ...
  // Main timing loop - running at 16 Hz
  ...
  if (currentMillis - freqmeasureMillis >= SAMPLE_PERIOD) {
    raw_frequency_meas = gateCounter;
    angular_frequency_pid = count;
    
    //raw_frequency_meas = sum / count; // FreqMeasure.countToFrequency(sum / count);
    ...
    if ((driver_state == ENABLE) && (change_pull_freq == LOW)) {
